...
def training(model, rounds, batch_size, lr, ds, data_dict, C, K, E, num_malicious, plt_title, plt_color, svhn_data_test,
    ...
    for curr_round in range(1, rounds+1):
        ...
        if curr_round % buffer_size != 0:
            ...
            predict_labels = FL_to_Grad_CAM(new_local_models, curr_round)  # elements in list is 0 or 1
            buffer.append(predict_labels)
            benign_idx_set = [index for index, value in enumerate(predict_labels) if value == 1]

        else:
            predict_labels = FL_to_Grad_CAM(new_local_models, curr_round)
            buffer.append(predict_labels)
            # sum buffer by row
        ...
        t_accuracy, t_loss = testing(model, svhn_data_test, test_batch_size, criterion, num_classes)
...
if __name__ == '__main__':
    ...
    num_clients = 18
    num_malicious = 2
    num_epochs = 25  # the number of local clients training epochs #25,
    client_lr = 2e-5  # client learning rate #2e-5,
    alpha_partition = 0.5
